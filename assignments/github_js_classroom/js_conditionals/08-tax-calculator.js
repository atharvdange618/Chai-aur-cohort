...
export function calculateTax(income) {
  ...
  let totalTaxAmountOwed = 0;

  if (income > 10000) {
    const taxableInBracket2 = Math.min(income, 30000) - 10000;
    totalTaxAmountOwed += taxableInBracket2 * 0.1;
  }

  if (income > 30000) {
    const taxableInBracket3 = Math.min(income, 70000) - 30000;
    totalTaxAmountOwed += taxableInBracket3 * 0.2;
  }

  if (income > 70000) {
    const taxableInBracket4 = income - 70000;
    totalTaxAmountOwed += taxableInBracket4 * 0.3;
  }

  return totalTaxAmountOwed;
}
